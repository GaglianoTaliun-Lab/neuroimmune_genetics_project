...
chunk_end <- as.numeric(args[4])

# list lava sumstats files for eQTLs
eqtls <- list.files(here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","onek1k"), recursive = T, full.names = T, pattern = str_c("^",cell_type_eqtl))
...
args <-
  list(
    ...
    loc_file = here(project_dir,"test_loci","window_100000","onek1k_filtered.loci"),
    info_file = here(project_dir,"info_files", "input.info.onek1k_gwas.txt"),
...
loci <-
  loci_all[c(chunk_start:chunk_end),]
...
saveRDS(
  univar,
  file = here(project_dir,"lava_results","lava_onek1k_QTLs","univ", str_c("GWAS_QTLs_",cell_type_eqtl, "_chunk_",chunk_name,".univ.lava.rds"))
)
saveRDS(
  bivar,
  file = here(project_dir,"lava_results","lava_onek1k_QTLs","bivar", str_c("GWAS_QTLs_",cell_type_eqtl, "_chunk_",chunk_name,".bivar.lava.rds"))
)

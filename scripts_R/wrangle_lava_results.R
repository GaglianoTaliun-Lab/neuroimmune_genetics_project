...
ntest_univ <- read.table(here(project_dir, "test_loci", "gwas_filtered.loci"), sep = "\t", header = T) %>% 
  nrow(.)
...
write.table(bivar_all, here(project_dir,"lava_results","tsv_univ_bivar_GWAS","phenotypes_all_results_bivar.tsv"), sep = "\t", quote = F, row.names = F)
write.table(univ_all, here(project_dir,"lava_results","tsv_univ_bivar_GWAS","phenotypes_all_results_univ.tsv"), sep = "\t", quote = F, row.names = F)

# write significant results into table
write.table(bivar_significant, here(project_dir,"lava_results","tsv_univ_bivar_GWAS","phenotypes_significant_results_bivar.tsv"), sep = "\t", quote = F, row.names = F)
write.table(univ_sign, here(project_dir,"lava_results","tsv_univ_bivar_GWAS","phenotypes_significant_results_univ.tsv"), sep = "\t", quote = F, row.names = F)

# Get file info file for LAVA  - for GWAS traits and OneK1K QTLs combined
...
QTL_names <- file_path_QTL %>%
	       mutate(
		 filename = 
                   stringr::str_remove(filename, here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","onek1k","")),
	         filename = 
...
rbind(input_info_GWAS, input_info_QTL, row.names = NULL) %>%
  write.table(
    .,
    here(project_dir,"info_files",stringr::str_c("input.info.onek1k_gwas.txt")),

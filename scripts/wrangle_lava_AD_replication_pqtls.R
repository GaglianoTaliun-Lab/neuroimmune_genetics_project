...
for(test in names(results)){
    ...
    setNames(
      object = 
        list.files(
          ...
          pattern = str_c("AD_replication.",test,".lava.rds"), 
          full.names = T
        ) %>% 
  ...
  cat("results_wrangled length = ", length(results_wrangled), ".\n")
    ...
    write.table(results_wrangled, here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("pQTLs_AD_replication_",test,"_",window,"_",date,".tsv")),
    ...
    results_wrangled %>% 
      ...
      write.table(., here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("bonferroni_significant_pQTLs_AD_replication_",test,"_",window,"_",date,".tsv")),
                  sep = "\t", quote=F, row.names=F)

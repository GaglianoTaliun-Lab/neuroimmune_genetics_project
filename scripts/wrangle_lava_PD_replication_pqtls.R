...
for(test in names(results)){
  ...
  cat("results_wrangled length = ", length(results_wrangled), ".\n")
    ...
    write.table(results_wrangled, here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("pQTLs_PD_replication_",test,"_",window,"_",date,".tsv")),
    ...
    results_wrangled %>% 
      ...
      write.table(., here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("bonferroni_significant_pQTLs_PD_replication_",test,"_",window,"_",date,".tsv")),
                  sep = "\t", quote=F, row.names=F)

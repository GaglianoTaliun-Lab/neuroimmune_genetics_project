# Description: script to wrangle results of LAVA with QTLs
...
for(test in names(results)){
  ...
  gene_filtered_loci <-
    readRDS(
      here(project_dir, "test_loci", window, "gene_filtered_loci.rds")
    )
  ...
  results[[test]][[window]] <- 
    setNames(
      object = 
        list.files(
          path = 
            here(project_dir,"lava_results","lava_QTLs",test),
          pattern = "GWAS_with_bulkQTL_significant*", 
          full.names = T
        ) %>% 
      ...
      nm = 
        ...
        str_remove(., ".lava.rds") %>%
        str_remove(., here(project_dir,"lava_results","lava_QTLs",test,"")) %>%
        str_remove(., "GWAS_with_QTL_") %>%
	str_replace(., "all_loci", "1MscBloodNL") %>%
	str_remove(., "[:punct:]\\d+[:punct:]\\d+")
    ) %>% 
  ...
  results_wrangled <-
    ...
    dplyr::select(
      ld_block, gene_locus, gene_name, everything()
    ) %>% 
    dplyr::arrange(ld_block, gene_locus) %>% 
    as_tibble() %>% unique()
  
  write.table(results_wrangled, here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("all_bulkQTL_",test,"_",window,"_",date,".tsv")),
    ...
    results_wrangled %>% 
      ...
      write.table(., here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("bonferroni_significant_bulkQTL_",test,"_",window,"_",date,".tsv")),
                  sep = "\t", quote=F, row.names=F)
    ...
    results_wrangled %>%
      ...
      write.table(., here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("FDR_significant_bulkQTL_",test,"_",window,"_",date,".tsv")),
                  sep = "\t", quote=F, row.names=F)

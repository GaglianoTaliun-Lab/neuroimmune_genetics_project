# Description: script to wrangle results of LAVA with 1MscBloodNL treatments QTLs
...
for(test in names(results)){
  ...
  gene_filtered_loci <-
    readRDS(
      here(project_dir, "test_loci", window, "1Msc_treatments_filtered_loci.rds")
    )
  ...
  results[[test]][[window]] <- 
    setNames(
      object = 
        list.files(
          path = 
            here(project_dir,"lava_results","lava_1Msc_treatments",test),
          pattern = ".lava.rds", 
          full.names = T
        ) %>% 
      ...
      nm = 
        ...
        str_remove(., ".lava.rds") %>%
        str_remove(., here(project_dir,"lava_results","lava_1Msc_treatments",test,"")) %>%
        str_remove(., "GWAS_with_") %>%
        str_remove(., "1MscBloodNL_chunk_") %>%
  ...
  if (test == "univ") {
    ...
    write.table(results_wrangled, here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("1MscBloodNL_treatments_",test,"_",window,"_",date,".tsv")),
    ...
    results_wrangled %>% 
      ...
      write.table(., here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("bonferroni_significant_1MscBloodNL_treatments_",test,"_",window,"_",date,".tsv")),
                  sep = "\t", quote=F, row.names=F)
    ...
    results_wrangled %>%
      ...
      write.table(., here(project_dir,"lava_results","tsv_univ_bivar_QTLs",stringr::str_c("FDR_significant_1MscBloodNL_treatments_",test,"_",window,"_",date,".tsv")),
                  sep = "\t", quote=F, row.names=F)

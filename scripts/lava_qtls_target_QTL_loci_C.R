...
# number of eqtl in array to run - set on SLURM array: 1-5 (Bcells, monocytes, CD4+ T cells, CD8+ T cells, NK cells)
n_qtl <- as.numeric(args[1])

# parallelisation chunk
chunk <- c(201:337)
chunk_name <- "201:337"

# array of all cell types (corresponding to the number of SLURM arrays)
cell_types <- c("Bcells", "CD4Tcells", "CD8Tcells", "monocyte", "NKcells")

# cell type to test in current array:
cell_type_eqtl <- cell_types[n_qtl]

cat("The qtl number in the array is: ", n_qtl,", which corresponds to ",cell_type_eqtl,". \n")

# list lava sumstats files for eQTLs
eqtls <- list.files(here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","1MscBloodNL"), recursive = T, full.names = T, pattern = str_c("^",cell_type_eqtl))

# remove path and suffix from list of eqtls
eqtl_names <- eqtls %>%
  stringr::str_remove(., here(project_dir, "GWAS_summary_statistics","LAVA_sumstats","1MscBloodNL","")) %>%
...
args <-
  list(
    ...
    loc_file = here(project_dir,"test_loci","window_100000","gene_filtered.loci"),
    info_file = here(project_dir,"info_files", "input.info.gene_gwas.txt"),
    sample_overlap_file = here(project_dir, "sample_overlap","gene_gwas_sample_overlap.txt"),
...
for (i in 1:nrow(loci)) {
  ...
  eqtl_gene <- str_c(cell_type_eqtl, "_1MscBloodNL_", gene)
...
saveRDS(
  univar,
  file = here(project_dir,"lava_results","lava_QTLs","univ", str_c("GWAS_with_QTL_",cell_type_eqtl, "_all_loci_",chunk_name,".univ.lava.rds"))
)
saveRDS(
  bivar,
  file = here(project_dir,"lava_results","lava_QTLs","bivar", str_c("GWAS_with_QTL_",cell_type_eqtl, "_all_loci_",chunk_name,".bivar.lava.rds"))
)

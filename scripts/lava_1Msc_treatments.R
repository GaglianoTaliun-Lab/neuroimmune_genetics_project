...
# treatment to test in current array:
treatment_type <- args[1]
...
eqtls <- list.files(here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","1MscBloodNL_treatments"), recursive = T, full.names = T, pattern = str_c("^",treatment_type))

# remove path and suffix from list of eqtls
eqtl_names <- eqtls %>%
  stringr::str_remove(., here(project_dir, "GWAS_summary_statistics","LAVA_sumstats","1MscBloodNL_treatments","")) %>%
...
args <-
  list(
    ...
    loc_file = here(project_dir,"test_loci","window_100000", "1Msc_treatments_filtered.loci"),
    info_file = here(project_dir,"info_files", "input.info.1Msc.treatments.txt"),
...
for (i in 1:nrow(loci)) {
  ...
  eqtl_gene <- str_c(treatment_type, "_", gene)
...
saveRDS(
  univar,
  file = here(project_dir,"lava_results","lava_1Msc_treatments","univ", str_c("GWAS_with_",treatment_type, "_chunk_",chunk_name,".univ.lava.rds"))
)
saveRDS(
  bivar,
  file = here(project_dir,"lava_results","lava_1Msc_treatments","bivar", str_c("GWAS_with_",treatment_type, "_chunk_",chunk_name,".bivar.lava.rds"))
)

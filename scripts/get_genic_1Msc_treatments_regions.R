...
unique_genes <-
  list.files(
    path = here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","1MscBloodNL_treatments"),
    pattern = "*.lava.gz",
  ) %>%
  stringr::str_remove_all(., ".lava.gz") %>%
  stringr::str_remove_all(., "[:alnum:]+_[:alnum:]+_1MscBloodNL_") %>%
  unique(.)
...
write.table(unique_genes, here(project_dir, "test_loci", "window_100000","unique_1Msc_treatments_genes.txt"), sep = "\t", row.names = F, quote = F, col.names = F)
...
write_delim(
  loci,
  delim = "\t",
  file = here(project_dir, "test_loci", "window_100000", "1Msc_treatments_filtered.loci")
)

saveRDS(
  overlap_df,
  file = here(project_dir, "test_loci", "window_100000", "1Msc_treatments_filtered_loci.rds")
)

...
phenotypes <- read.table(here(project_dir, "GWAS_summary_statistics","list_of_GWAS_studies.txt"), sep = "\t", header = F) %>% .[,1] %>% as.array() %>% sort()

args <-
  list(
    ref_prefix = here(project_dir,"reference_data","g1000_eur","g1000_eur"),
    loc_file = here(project_dir,"test_loci","gwas_filtered.loci"),
    info_file = here(project_dir,"info_files","input.info.txt"),
    sample_overlap_file = here(project_dir,"sample_overlap","sample_overlap.txt"),

...
bivar_all <- read.table(here(project_dir,"lava_results", "tsv_univ_bivar_GWAS","phenotypes_all_results_no_proxies_bivar.tsv"), sep = "\t", header = T)
bivar_bonf <- read.table(here(project_dir,"lava_results","tsv_univ_bivar_GWAS","phenotypes_significant_results_no_proxies_bivar.tsv"), sep = "\t", header = T)
...
bivar_bonf %>% 
  ...
  dplyr::inner_join(
    read.table(
      here(project_dir,"info_files","input.info.no_proxies.txt"), sep = "\t", header = T) %>% 
      dplyr::mutate(
...
ggsave(here(project_dir, "lava_results", "figures", "phenotypes_scatter_plot_no_proxies.pdf"), width = 40, height = 30, units = "cm")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#### Chord diagram:
...
ggsave(here(project_dir, "lava_results", "figures", "phenotypes_heatmaps_per_LDblock_no_proxies.pdf"), width = 35, height = 65, units = "cm")
...
ggsave(here(project_dir, "lava_results", "figures", "phenotypes_edge_diagram_no_proxies.pdf"), width = 40, height = 50, units = "cm")
...
pdf(here(project_dir, "lava_results", "figures", "phenotypes_LDblock_annotations_no_proxies.pdf"))
fig_list
dev.off()

...
file_path_QTL <- data.frame(filename = list.files(
    path = here(project_dir, "GWAS_summary_statistics", "LAVA_sumstats", "eQTLGen"),
    full.names = T,
    recursive = T,
    pattern = ".lava.gz"
  )
...
QTL_names <- file_path_QTL %>%
	       mutate(
		 filename = 
                   stringr::str_remove(filename, here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","eQTLGen","")),
	         filename = 
...
rbind(input_info_GWAS, input_info_QTL, row.names = NULL) %>%
  write.table(
    .,
    here(project_dir,"info_files",stringr::str_c("input.info.bulkQTL_gwas.txt")),
    sep = "\t", row.names = F, quote = F
  )

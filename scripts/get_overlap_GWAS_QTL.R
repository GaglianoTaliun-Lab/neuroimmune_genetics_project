...
# GWAS phenotypes to include in the LAVA analysis with QTLs
gwas <- array(args[1:length(args)]) %>% sort()
...
eqtl_gene <-
  list.files(
    path = here(project_dir, "GWAS_summary_statistics","LAVA_sumstats","QTLs"),
    pattern = ".lava.gz"
  ) %>%
  basename() %>%
  str_remove(".lava.gz")
...
gwas_rg <-
  read.table(
    file = here(project_dir,"ldsc_corr","ldsc_correlations.v3.txt"),
    sep = "\t", header = T
  )

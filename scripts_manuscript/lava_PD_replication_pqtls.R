# LAVA QTL FLD
# Description: run univariate and bivariate tests for pQTL/GWAS traits for replication of AD signals
# runs the univariate and bivariate tests together for each GWAS/eGene separately, using as target the qtl in question (arrays).
...
eqtls <- list.files(here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","pqtls"), recursive = T, full.names = T, pattern = "*.lava.gz")

# remove path and suffix from list of eqtls
eqtl_names <- eqtls %>%
  stringr::str_remove(., here(project_dir, "GWAS_summary_statistics","LAVA_sumstats","pqtls","")) %>%
...
args <-
  list(
    ...
    loc_file = here(project_dir,"test_loci","window_100000", "PD_replication_pqtls.loci"),
    info_file = here(project_dir,"info_files", "input.info.replication.PD.pqtls.txt"),
...
saveRDS(
  univar,
  file = here(project_dir,"lava_results","lava_pqtls","univ", str_c("GWAS_with_pqtls_PD_replication.univ.lava.rds"))
)
saveRDS(
  bivar,
  file = here(project_dir,"lava_results","lava_pqtls","bivar", str_c("GWAS_with_pqtls_PD_replication.bivar.lava.rds"))
)

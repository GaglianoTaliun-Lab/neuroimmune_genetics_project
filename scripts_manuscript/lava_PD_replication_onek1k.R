# LAVA QTL FLD
# Description: run univariate and bivariate tests for eQTL/GWAS traits as a replication for PD signals
...
# number of eqtl in array to run - set manually on shell script argument: (Bcells [2], monocytes [1], CD4+ T cells [2], CD8+ T cells [2])
cell_type_eqtl <- args[1]

# list lava sumstats files for eQTLs
eqtls <- list.files(here(project_dir,"GWAS_summary_statistics","LAVA_sumstats","onek1k"), recursive = T, full.names = T, pattern = stringr::str_c("^", cell_type_eqtl))

# remove path and suffix from list of eqtls
eqtl_names <- eqtls %>%
  stringr::str_remove(., here(project_dir, "GWAS_summary_statistics","LAVA_sumstats","onek1k","")) %>%
...
args <-
  list(
    ...
    loc_file = here(project_dir,"test_loci","window_100000","PD_replication_onek1k.loci"),
    info_file = here(project_dir,"info_files", "input.info.replication.PD.onek1k.txt"),
    gwas_info = here(project_dir, "info_files", "input.info.replication.PD.txt"),
    phenotypes = c(""),
...
cat("The gwas phenotypes are: ", gwas, ". \n")
...
for (i in 1:nrow(loci)) {
  ...
  eqtl_gene <- str_c(cell_type_eqtl, "_onek1k_", gene)
  
  cat("The eqtl_gene pair is: ",eqtl_gene,". \n")
  
  # check if gene is present in that cell type
  if (!eqtl_gene %in% eqtl_names){
    cat("Chunk", i," -- no gene-eqtl pair for", eqtl_gene, "\n")
    next
  }
  
  cat("Chunk", i, "-- Will start processing locus for", eqtl_gene,"\n")
  ...
  for (j in 1:length(gwas)){
    ...
    if (!is.null(locus)) {
      ...
      bivar_one_loop[[j]] <- loc_info %>%
        ...
        cat("Chunk ", i, " -- no LAVA test run for GWAS ",gwas_to_test, " on ", eqtl_gene, "due to null locus.\n")
...
saveRDS(
  univar,
  file = here(project_dir,"lava_results","lava_onek1k_QTLs","univ", str_c("GWAS_QTLs_",cell_type_eqtl, "_PD_replication.univ.lava.rds"))
)
saveRDS(
  bivar,
  file = here(project_dir,"lava_results","lava_onek1k_QTLs","bivar", str_c("GWAS_QTLs_",cell_type_eqtl, "_PD_replication.bivar.lava.rds"))
)
